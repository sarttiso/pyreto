...
class FitParetoGen:
    def __init__(self, X):
        ...
        Using notation of Choulakian and Stephens (2001), 
        F(x) = 1 - (1 - kx/a)^1/k
        where a is the scale parameter and k is the shape parameter.
        0 <= x <= infinity for k <= 0, 0 <= x <= a/k for k > 0
    ...
    def loglike(self, theta):
        ...
        ----------
        theta : float
            Maximization variable, corresponds to k/a
        ...
        float
            log likelihood
        """
        return -self.n - np.sum(np.log(1 - theta*self.X)) - self.n * np.log(-1/(self.n*theta) * np.sum(np.log(1-self.X*theta)))
    ...
    def fit(self):
        ...
        with warnings.catch_warnings():
            ...
            sol = minimize_scalar(lambda x: -self.loglike(x), bounds=[-1e10, np.max(1/self.X)], method='bounded')
        theta_hat = sol.x
        k_hat = -1/self.n * np.sum(np.log(1-self.X*theta_hat))
        a_hat = k_hat/theta_hat
...
class FitParetoTrunc:
    ...
    def fit(self, method='Beg'):
        """Fit the truncated Pareto distribution.

        Parameters
        ----------
        method : str, optional
            Method to use, by default 'Beg'

        Returns
        -------
        float, float
            nu (truncation parameter), alpha (power law exponent)
        """
        if method=='Beg':
            return self.nu(method), self.alpha(method)
        elif method=='Aban':
            return self.nu(method), self.alpha(method, r=0)
...
def pareto_gen_pdf(x, k, a):
    ...
    if k <= 0:
        ...
        assert np.all(x >= 0) and np.all(x <= a/k), "x must be between 0 and a/k"
    p = (1/a) * (1 - k*x/a)**((1-k)/k)
    return p

def pareto_gen_cdf(x, k, a):
